...
pub fn sign_call_with_keypair(
    ...
    let signature = sign_payload(keypair, &payload, ctx.spec_version)?;
...
fn sign_payload(keypair: &ml_dsa_87::Keypair, payload: &[u8], spec_version: u32) -> Result<Vec<u8>, Error> {
    // Quantus runtime >= 148 uses FIPS 204 extrinsic domain separation.
    let context = (spec_version >= 148).then_some(b"QUANTUS_EXTRINSIC".as_slice());
...
mod tests {
    ...
    fn signature_field_matches_qp_dilithium_crypto() {
        ...
        let sig = sign_payload(&keypair, msg, 147).unwrap();
    ...
    fn signed_transfer_signature_verifies() {
        let seed = [0u8; 32];
        for spec in [147, 148, 152] {
        let mut p = sample_params(None);
        p.ctx.spec_version = spec;
        ...
        let verifier = quantus_modern_dilithium::ml_dsa_87::PublicKey::from_bytes(&keys.public_key).unwrap();
        let context = (spec >= 148).then_some(b"QUANTUS_EXTRINSIC".as_slice());
        assert!(verifier.verify(&signable, sig_bytes, context));
        if spec >= 148 { assert!(!verifier.verify(&signable, sig_bytes, None)); }
        }
    }

    #[test]
    fn runtime_domain_is_required_for_short_and_hashed_payloads() {
        let keypair = keypair_from_seed(&[7u8; 32]).unwrap();
        for payload in [vec![42; 100], vec![42; 300]] {
            let message = if payload.len() > PAYLOAD_HASH_THRESHOLD {
                blake2_256(&payload).to_vec()
            } else { payload.clone() };
            for spec in [148, 152] {
                let signature = sign_payload(&keypair, &payload, spec).unwrap();
                assert!(keypair.verify(&message, &signature, Some(b"QUANTUS_EXTRINSIC")));
                let modern = quantus_modern_dilithium::ml_dsa_87::PublicKey::from_bytes(&keypair.public.to_bytes()).unwrap();
                assert!(modern.verify(&message, &signature, Some(b"QUANTUS_EXTRINSIC")));
                assert!(!modern.verify(&message, &signature, None));
                assert!(!keypair.verify(&message, &signature, None));
                assert!(!keypair.verify(&message, &signature, Some(b"OTHER_DOMAIN")));
            }
            let legacy = sign_payload(&keypair, &payload, 147).unwrap();
            assert!(keypair.verify(&message, &legacy, None));
            assert!(!keypair.verify(&message, &legacy, Some(b"QUANTUS_EXTRINSIC")));
        }
    }
